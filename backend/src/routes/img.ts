 ...
 */
const imgRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.get('/img', async (req: FastifyRequest, reply: FastifyReply) => {
    const { u } = req.query as { u?: string }
    if (!u) return reply.code(400).send({ error: 'Parâmetro "u" ausente.' })
