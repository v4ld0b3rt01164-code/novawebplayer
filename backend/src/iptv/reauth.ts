 ...
 */
const inFlightReauths = new Map<string, Promise<ActiveServer>>()
 ...
 */
export async function reauthenticateSession(session: Session): Promise<ActiveServer> {
  const existing = inFlightReauths.get(session.token)
  if (existing) return existing

  const attempt = (async (): Promise<ActiveServer> => {
    session.blockedServers.add(session.server.baseUrl)

    try {
      const { server } = await authenticate(
        session.server.username,
        session.server.password,
        session.blockedServers,
      )
      updateSessionServer(session.token, server)
      console.log(
        `[reauth] sessão ${session.token.slice(0, 8)}… migrada para novo servidor upstream`,
      )
      return server
    } catch (err) {
