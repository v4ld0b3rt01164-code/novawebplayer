...
const authRoutes = async (app) => {
    app.post('/auth', {
        ...
        try {
            ...
            console.log(`[auth] login OK — servidor ativo: ${new URL(server.baseUrl).host} (sessão ${token.slice(0, 8)}…)`);
            return reply.send({
                token,
            ...
            return reply.status(401).send({
                error: 'Não foi possível conectar a nenhum servidor. Verifique usuário/senha ou tente novamente mais tarde.',
            });
        }
    });
};
