...
function buildUpstreamBase(session, type) {
...
export function buildUpstreamUrl(session, type, file) {
 ...
 */
const realServerCache = new Map();
async function discoverRealServer(session, type, file) {
    ...
    if (res.status === 401 || res.status === 403) {
        throw new UpstreamHttpError(res.status, `Descoberta de servidor real bloqueada (${res.status}) em ${cacheKey}`);
    }
...
export async function fetchRewrittenPlaylist(session, type, file, backendOrigin, token) {
...
export async function fetchUpstreamRaw(upstreamUrl, rangeHeader) {
    const headers = {
